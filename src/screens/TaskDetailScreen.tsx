...
export default function TaskDetailScreen({ onBack, taskId }: Props) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      const generalRecipients = allInterestedUsers.filter(userId => userId !== currentUser.uid);

      const membersToMentionInComment = projectMemberIds
        .map(id => usersMap[id])
        .filter((u: any) => 
          u && u.uid !== currentUser?.uid && commentText.includes(`@${u.fullName}`)
        );
  ...
  const handleCyclePriority = async () => {
    ...
    try {
      const taskRef = doc(db, 'tasks', taskId);
      await updateDoc(taskRef, { priority: priorityMap[task.priority] || 'Med' });
    } catch (error) {
  ...
  const filteredMembersToMention = projectMemberIds
    .map((id) => usersMap[id])
    .filter((u: any) => 
      u && u.uid !== currentUser?.uid && u.fullName?.toLowerCase().includes(mentionQuery.toLowerCase())
    );

    const renderFormattedComment = (text: string) => {
    const memberNames = projectMemberIds.map((id) => usersMap[id]?.fullName).filter((v): v is string => !!v);
    ...
    return words.map((word, index) => {
      ...
      if (isExactMention || (word.startsWith('@') && memberNames.some(name => word.substring(1).includes(name)))) {
        return (
    ...
    >
      <SafeAreaView style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          {/* Activity / Comments */}
          <View style={styles.sectionPadding}>
            <Text style={styles.sectionHeader}>ACTIVITY ({comments.length})</Text>
            {comments.length === 0 ? (
...
{showMentionsList && filteredMembersToMention.length > 0 && (
  <View style={styles.mentionsDropdown}>
    {filteredMembersToMention.map((member: any) => (
      ...
      >
        <View style={[styles.avatar, { backgroundColor: '#566551', width: 24, height: 24 }]}>
          <Text style={[styles.avatarText, { fontSize: 8 }]}>{getInitials(member.fullName)}</Text>
        </View>
