...
export default function SearchScreen({ onBack }: Props) {
  ...
  const displayedProjects =
    filter === "all"
      ? filteredProjects.slice(0, PREVIEW_LIMIT)
      : filteredProjects;
  const displayedTasks =
    filter === "all" ? filteredTasks.slice(0, PREVIEW_LIMIT) : filteredTasks;
  const displayedFiles =
    filter === "all" ? filteredFiles.slice(0, PREVIEW_LIMIT) : filteredFiles;
        ...
        {showFiles && (
          <View style={styles.section}>
            ...
            {displayedFiles.map((file, idx) => {
              const taskTitle = file.taskId
                ? taskTitleMap.get(file.taskId)
                : undefined;
              return (
